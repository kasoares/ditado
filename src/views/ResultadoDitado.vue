<template>
  <div>
    ...
    <div v-else-if="detalhes">
      ...
      <v-card class="mb-6" elevation="1">
        <v-card-title class="pa-4 d-flex align-center border-b bg-amber-lighten-5">
          <v-icon color="warning" class="mr-2">mdi-alert-circle</v-icon>
          Erros por Tipo
        </v-card-title>
        <v-card-text class="pa-6 bg-grey-darken-4">
          <div v-if="detalhes.errosPorTipo && detalhes.errosPorTipo.length > 0">
            <v-row v-for="erro in detalhes.errosPorTipo" :key="erro.tipoErroId" align="center" no-gutters class="mb-2">
              <v-col cols="4" sm="3" class="text-right pr-4 text-white font-weight-bold">
                {{ erro.descricaoCurta }}:
              </v-col>
              <v-col cols="6" sm="8">
                <v-progress-linear
                  :model-value="(erro.quantidade / maxErros) * 100"
                  color="white"
                  height="24"
                  rounded
                ></v-progress-linear>
              </v-col>
              <v-col cols="2" sm="1" class="pl-3 text-white font-weight-black text-h6">
                {{ erro.quantidade }}
              </v-col>
            </v-row>
          </div>
...
// Calcula o máximo para a barra de progresso (escala do gráfico)
const maxErros = computed(() => {
  if (!detalhes.value?.errosPorTipo?.length) return 1
  return Math.max(...detalhes.value.errosPorTipo.map(e => e.quantidade))
})
